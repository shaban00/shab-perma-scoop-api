...
def config_check() -> bool:
    ...
    for prop in ["API_DOMAIN", "API_STORAGE_URL"]:
    ...
    ua_config = config["CUSTOM_USER_AGENT_DOMAINS"]
    if ua_config:
        for domain, agents in ua_config.items():
            if not isinstance(agents, dict):
                raise Exception(f"Custom user agent config for {domain} should be a dictionary.")
            for key in agents:
                if key not in ["validator_ua", "scoop_ua_suffix"]:
                    raise Exception(
                        "Custom user agent allowed keys: validator_ua, scoop_ua_suffix."
                    )
