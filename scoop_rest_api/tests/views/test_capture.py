...
@patch("scoop_rest_api.views.capture.start_capture_process")
def test_capture_post_save(start_capture_process, client, access_key, default_capture_url):
    ...
    total_before_request = Capture.select().order_by(Capture.id_capture).count()
    ...
    total_after_request = Capture.select().order_by(Capture.id_capture).count()
    ...
    start_capture_process.delay.assert_called_once()
...
def test_capture_get_id_capture_failed_job(
    client, runner, access_key, large_capture_url, id_capture_large, caplog
):
    """[GET] /capture returns HTTP 200 when provided with a valid id_capture."""
    from scoop_rest_api.tasks import start_capture_process
    ...
    # Check output before capture is run
    response = client.get(
        f"/capture/{id_capture_large}", headers={"Access-Key": access_key_readable}
    )
    response_data = response.get_json()

    assert response.status_code == 200
    assert "id_capture" in response_data
    assert response_data["url"] == large_capture_url
    assert response_data["status"] == "pending"
    assert "stderr_logs" not in response_data
    assert "stdout_logs" not in response_data
    assert "scoop_capture_summary" not in response_data
    ...
    assert "Failed (Archive over maximum supported filesize)" in caplog.records[-1].msg
